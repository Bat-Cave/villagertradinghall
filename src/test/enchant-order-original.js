...
export function runEnchantProcess({ item, enchants, mode = 'levels', data }) {
    if (data && data.ids && data.weights) {
        ID_LIST = data.ids;
        ENCHANTMENT2WEIGHT = data.weights;
    } else if (data && data.weightsByName) {
        // Alternate shape: { weightsByName: Record<string, number> }
        ID_LIST = {};
        ENCHANTMENT2WEIGHT = [];
        let id = 0;
    ...
    return processMain(item, enchants, mode);
}

function processMain(item, enchants, mode = 'levels') {
    ITEM_NAME = item;
    Object.freeze(ITEM_NAME);

    let enchant_objs = [];
    enchants.forEach(enchant => { // [name, level]
    ...
    if (ITEM_NAME === 'book') {
        ...
        item = new item_obj(id, enchant_objs[mostExpensive].l); // base is the most expensive book
        item.e.push(id);
    ...
    for (const key in cheapest_items) {
        const candidate = cheapest_items[key];
        let item_cost = mode === 'levels' ? candidate.x : candidate.w;
...
const memoizeCheapest = (func) => {
    return (...args) => {
        const args_key = memoizeHashFromArgs(args);
        if (!results[args_key]) {
